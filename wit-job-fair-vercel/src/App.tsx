...
function normalizeJob(j: any): Job {
  return {
    ...
    tags: Array.isArray(j.tags)
      ...
      ? j.tags.split(",").map((x: string) => x.trim()).filter(Boolean)
      : [],
...
export default function JobFairLanding() {
  ...
  function exportJSON() {
    ...
    a.href = url;
    a.download = `job-fair-vacancies-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
            ...
            <div>
              <div className="flex items-center gap-3">
                <a href="https://www.iq.zain.com" target="_blank" rel="noopener noreferrer">
                  <img src={ZAIN_LOGO_URL} alt="Zain Iraq" className="h-10 w-auto" />
                </a>
      ...
      {admin ? (
        <section className="mx-auto max-w-7xl px-6 pb-6">
          <div className="grid gap-3 md:grid-cols-3">
            <div className="md:col-span-2 flex flex-wrap items-center gap-3">
              ...
              <details className="rounded-2xl border border-gray-200 bg-white px-4 py-2 text-sm">
                ...
                <div className="mt-3 flex gap-2">
                  <input
                    id="sheet-url"
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:border-purple-500"
            ...
            <div className="text-sm text-gray-500">
              Admin only. Import JSON/CSV with: id, title, company, location, type, tags (comma), description, responsibilities (semicolon/newline), requirements (semicolon/newline), applyLink (optional).
            </div>
          </div>
        </section>
      ) : (
        <section className="mx-auto max-w-7xl px-6 pb-6">
          <div className="rounded-2xl border border-dashed border-gray-300 p-4 text-sm text-gray-600">
            Organizers:{" "}
            <button onClick={() => setShowAdminLogin(true)} className="underline">
              sign in
            </button>{" "}
            to add or import vacancies.
          </div>
        </section>
      )}
...
function AddVacancyModal({
  ...
  onClose: () => void;
  onSave: (jobDraft: Partial<Job>) => void;
}) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain px-6 pt-4 pb-28">
          <form id={formId} onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <label className="mb-1 block text-sm font-medium">External Apply Link (optional)</label>
              <input className="w-full rounded-xl border border-gray-200 px-3 py-2" value={form.applyLink} onChange={set("applyLink")} placeholder="https://airtable.com/shr... or https://tally.so/r/..." />
            </div>
            {error && (
...
function AdminLoginModal({
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    const PASS =
      (typeof window !== "undefined" && (window as any).ADMIN_PASSCODE) || "ZAIN-ADMIN";
    if (code.trim() === PASS) {
      setError("");
      onSuccess?.();
    } else {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <motion.div
        initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}
        className="relative z-10 w-full max-w-md rounded-3xl bg-white p-6 shadow-xl max-h-[80vh] overflow-y-auto"
      >
...
function JobModal({
  ...
  function validate(): string {
    if (job.applyLink) return ""; // external form mode
    ...
    try {
      ...
      if (job.applyLink) {
        const url =
          ...
          `&tags=${encodeURIComponent((job.tags || []).join(","))}`;
            ...
            {job.applyLink ? (
              <div className="rounded-2xl border border-purple-200/60 bg-purple-50 p-4">
                ...
                <a
                  ...
                  className="inline-flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-purple-600 to-fuchsia-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:opacity-95"
                >
